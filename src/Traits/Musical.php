...
trait Musical
{
    ...
    protected function tencent_url($result)
    {
        ...
        foreach ($type as $index => $vo) {
            if ($data['data'][0]['file'][$vo[0]] && $vo[1] <= ArrayHelper::get($data, 'api.br')) {
    ...
    protected function xiami_url($result)
    {
        ...
        foreach ($data['data']['data']['songs'][0]['listenFiles'] as $vo) {
            if ($type[$vo['quality']] <= ArrayHelper::get($data, 'api.br') && $type[$vo['quality']] > $max) {
                $max = $type[$vo['quality']];
                $url = [
                    'url' => $vo['listenFile'],
                    'size' => $vo['fileSize'],
    ...
    protected function kugou_url($result)
    {
        ...
        foreach ($data['data'][0]['relate_goods'] as $vo) {
            if ($vo['info']['bitrate'] <= ArrayHelper::get($data, 'api.br') && $vo['info']['bitrate'] > $max) {
                $api = [
                    'method'    => 'GET',
    ...
    protected function baidu_url($result)
    {
        ...
        if (isset($data['songurl']['url'])) {
            foreach ($data['songurl']['url'] as $vo) {
                if ($vo['file_bitrate'] <= ArrayHelper::get($data, 'api.br') && $vo['file_bitrate'] > $max) {
                    $url = [
                        'url'   => $vo['file_link'],
                        'br'    => $vo['file_bitrate'],
                    ];
